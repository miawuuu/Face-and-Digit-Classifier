...
def load_data(images_file, labels_file, width=28, height=28):
...
def counters_to_matrix(data, width=28, height=28):
    N = len(data)
    D = width * height
...
def evaluate_model(model_type, train_data, train_labels, test_data, test_labels, X_test, X_val, y_val):
    ...
    for pct in percentages:
        ...
        predictions_for_display = None  # To store predictions from the last run

        for run in range(5):
            # Randomly select a subset of training data based on the percentage
            subset_size = int(len(train_data) * pct / 100) # subset size based on percentage
            indices = random.sample(range(len(train_data)), subset_size) # randomly select indices for the subset
            data_subset = [train_data[i] for i in indices] 
            labels_subset = [train_labels[i] for i in indices] 
            
            # Convert the subset to matrix form
            X_train = counters_to_matrix(data_subset)
            y_train = np.array(labels_subset)

            # If model_type is perceptron
            if model_type == 'perceptron':
                classifier = PerceptronClassifier(legalLabels=list(range(10)), max_iterations=3)
                start = time.time()
                classifier.train(data_subset, labels_subset, [], [])
                end = time.time()
                predictions = classifier.classify(test_data)

            # If model_type is nn_scratch
            elif model_type == 'nn_scratch':
                classifier = ThreeLayerNet(X_train.shape[1], 128, 64, 10)
                start = time.time()
                classifier.train(X_train, y_train, X_val, y_val, epochs=10)
                end = time.time()
                predictions = classifier.predict(X_test)

            # If model_type is pytorch
            ...
            predictions_for_display = predictions  # save last predictions

        # Calculate mean and std of accuracies and times
...
if __name__ == "__main__":
    ...
    train_data, train_labels = load_data('data/digitdata/trainingimages', 'data/digitdata/traininglabels')
    val_data, val_labels = load_data('data/digitdata/validationimages', 'data/digitdata/validationlabels')
    test_data, test_labels = load_data('data/digitdata/testimages', 'data/digitdata/testlabels')

    # Convert data from counters to matrices
    X_val = counters_to_matrix(val_data)
    X_test = counters_to_matrix(test_data)
